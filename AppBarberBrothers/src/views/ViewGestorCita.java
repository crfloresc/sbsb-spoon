...
public class ViewGestorCita extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        List<String> listaEmpleados = cmbEmpleados();
        String[] arrayEmpleados = new String[listaEmpleados.size()];
        ...
        List<String> listaServicios = cmbServicios();
        String[] arrayServicios = new String[listaServicios.size()];
        listaServicios.toArray(arrayServicios);
    ...
    /**
     * Aqui se ejecuta el agrega la cita
     * @param evt el click
     */
...
//            Servicios s = new Servicios(cbServicios.getSelectedIndex() + 1, cbServicios.getSelectedItem().toString(), 100.00, 2);
            Empleado b = encontrarEmpleado(cbBarberos.getSelectedItem().toString());
            Clientes c = new Clientes(cbClientes.getSelectedIndex() + 1, cbClientes.getSelectedItem().toString());
            Servicios s = encontrarServicio(cbServicios.getSelectedItem().toString());
            ...
            // System.out.println(b+","+c+","+f+","+s); // ONLY DEBUG

            try {
                // Aqui se agrega la cita
                boolean isAdded = ViewMenuCitas.controlCitas.agregar(ViewMenuCitas.controlCitas.listar().size(), f, b, c, s);
                ...
                if (isAdded) {
                    ...
                    javax.swing.JOptionPane.showMessageDialog(this, "La hora ya se encuentra elegida, elija otra", "Error", javax.swing.JOptionPane.WARNING_MESSAGE);
                }
            } catch (Exception ex) {
    ...
    private Empleado encontrarEmpleado(String nombre) {
        Empleado emp = new Empleado();
        for (Object obj : ViewMenuCitas.controlEmpleados.listar()) {
            emp = (Empleado) obj;
            if(emp.getNombreCompleto().equalsIgnoreCase(nombre)) {
                return emp;
            }
        }
        return emp;
    ...
    private Servicios encontrarServicio(String nombre) {
...
//            Servicios s = new Servicios(cbServicios.getSelectedIndex() + 1, cbServicios.getSelectedItem().toString(), 100.00, 2);
            ...
            try {
                // Aqui se modifica la cita y posteriormente se agrega al controlador
    ...
    private List<String> cmbEmpleados() {
        List<String> values = new ArrayList<>();

        for (Object obj : ViewMenuCitas.controlEmpleados.listar()) {
            Empleado emp = (Empleado) obj;
            values.add(emp.getNombreCompleto());
        }
        
        return values;
    }
    
    private List<String> cmbServicios() {
        List<String> values = new ArrayList<>();

        for (Object obj : ViewMenuCitas.controlServicios.listar()) {
            Servicios ser = (Servicios) obj;
            values.add(ser.getNombre());
        }
    ...
    private List<String> test() {
        ...
        defaultValues.add("10:00");
        defaultValues.add("11:00");
        defaultValues.add("12:00");
        defaultValues.add("13:00");
        defaultValues.add("14:00");
        defaultValues.add("15:00");
        defaultValues.add("16:00");
        defaultValues.add("17:00");
        defaultValues.add("18:00");
        defaultValues.add("19:00");
        defaultValues.add("20:00");

        /*ViewMenuCitas.controlCitas.listar().forEach(action -> {
            Citas cita = (Citas) action;
            ...
            if (realDay == day) {
                ...
                System.out.println("Dia " + day + " a las " + hour + ":" + minutes);

                for (int i = 0; i < cita.getServicio().getDuracion(); i++) {
                    defaultValues.remove((hour + i) + ":00");
                }
            }
        });*/
