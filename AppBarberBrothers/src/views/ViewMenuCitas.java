...
public class ViewMenuCitas extends javax.swing.JFrame {
    ...
    static ControlCitas controlCitas = new ControlCitas();
    static ControlEmpleados controlEmpleados = new ControlEmpleados();
    static ControlServicios controlServicios = new ControlServicios();
    ...
    private void initComponents() {
        ...
        tblSchedule = new components.table.TableSchedule(ViewMenuCitas.controlCitas.listar());
        ...
        pnlMainLayout.setVerticalGroup(
            ...
            .addGroup(pnlMainLayout.createSequentialGroup()
                ...
                .addGap(0, 0, 0)
                .addGroup(pnlMainLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addComponent(pnlHamburgerMenu, javax.swing.GroupLayout.DEFAULT_SIZE, 497, Short.MAX_VALUE)
                    .addComponent(pnlScheduleSection, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                    .addGroup(pnlMainLayout.createSequentialGroup()
    ...
    private void btnModificarCitaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnModificarCitaActionPerformed
        ...
        if (row != -1) {
            ...
            String value = String.valueOf(model.getValueAt(row, 0)) ;
            System.out.println("ID Cita: " + value);

            ViewGestorCita vgc = new ViewGestorCita(ViewGestorCita.MODIFICAR);
            vgc.enviarCitaModificar(Integer.valueOf(value));
            vgc.setVisible(true);
            this.dispose();
        } else {
        ...
        if (row != -1) {
            ...
            if (!value.isEmpty()) {
                ...
                if (confirm == javax.swing.JOptionPane.YES_OPTION) {                    
                    controlCitas.eliminar(Integer.valueOf(value));
                    model.removeRow(row);
    ...
    private void tblCalendarMouseClicked(java.awt.event.MouseEvent evt) {
        ...
        if (row >= 0 && col >= 0) {
            if (!value.contains("null")) {
                ...
                for (Object object : ViewMenuCitas.controlCitas.listar()) {
                    if (object instanceof Citas) {
                        Citas c = (Citas) object;
                        
                        Date day = new Date(c.getFecha().getYear()+1900, c.getFecha().getMonth(), c.getFecha().getDate());
                        if (day.equals( new java.util.Date(currentYear, currentMonth, Integer.valueOf(value)))) {
                            model.insertRow(rowCount, new Object[]{c.getIdCita(), c.getFecha(), c.getCliente().getNombre(), c.getBarbero().getNombreCompleto(), 
                                c.getServicio().getNombre()});
                            rowCount++;
                        }
                    } else {
                        throw new RuntimeException("No se obtuvieron los datos");
                    }
                }
            }
        }
